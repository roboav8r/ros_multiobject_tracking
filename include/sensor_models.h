...
namespace SensorModels {
    ...
    */
    class Position2DPoseArray
    {
        public:
            // Construct with sensor variance and detection probability
            Position2DPoseArray(float variance, float prob, float clutter_density, std::string tracker_frame): 
    ...
    class Position2DPersonArray
    {
        public:
            // Construct with sensor variance and detection probability
            Position2DPersonArray(float variance, float prob, float clutter_density, std::string tracker_frame): 
            _measVariance(variance), 
            _probDetect(prob),
            _clutterDensity(clutter_density),
            _trackerFrame(tracker_frame),
            ...
            // Members
            void MeasUpdateCallback (const leg_tracker::PersonArray::ConstPtr& msg, MultiObjectTrackers::GmPhdFilter2D* tracker)
            {
                std::unique_lock<std::mutex> ul(tracker->TrackerMutex, std::defer_lock);
...
class Position2DSpatialDetectionArray
    {
        public:
            // Construct with sensor variance and detection probability
            Position2DSpatialDetectionArray(float variance, float prob, float clutter_density, std::string tracker_frame, int human_label, float det_thresh): 
            _measVariance(variance), 
            _probDetect(prob),
            _clutterDensity(clutter_density),
            _trackerFrame(tracker_frame),
            ...
            _obsCovMatrix(Eigen::MatrixXf::Identity(2,2)),
            _listener(_tfBuffer),
            _humanLabel(human_label),
            _detectionThreshold(det_thresh)
            {
                _obsMatrix.topLeftCorner(2,2) = Eigen::MatrixXf::Identity(2,2);
                _obsCovMatrix = _obsCovMatrix*pow(variance,2);
            };
            ...
            // Members
            void MeasUpdateCallback (const depthai_ros_msgs::SpatialDetectionArray::ConstPtr& msg, MultiObjectTrackers::GmPhdFilter2D* tracker)
            {
                std::unique_lock<std::mutex> ul(tracker->TrackerMutex, std::defer_lock);
                ...
                if (tracker->ReadyToUpdate) {
                    ...
                    if (ul.try_lock()) {
                        ...
                        geometry_msgs::PoseStamped originalPose;
                        originalPose.header = msg->header;
                        _nDetections = msg->detections.size();

                        for (auto& detection : msg->detections ) // Iterate through detections
                        {

                            // If this is not a valid detection, skip it
                            if ( detection.results[0].id != _humanLabel || detection.results[0].score < _detectionThreshold) 
                            {
                                continue;
                            }
                            ...
                            originalPose.pose.position = detection.position;
                            originalPose.pose.position.y = -detection.position.y; // Fix OAK-D left handed coordinate convention
                            geometry_msgs::PoseStamped transformedPose;
